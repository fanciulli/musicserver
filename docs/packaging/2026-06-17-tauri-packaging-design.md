...
3. Keep all packaging code in the `musicserver` repository. The
   `musicserver-backend` and `musicserver-admin-ui` source repos needed only
   two minimal changes: the backend reads its listen port from `PORT` (§4.2),
   and the admin UI ties the session cookie's `Secure` flag to `HTTPS_ENABLED`
   so login works over the loopback HTTP UI.
...
│   └── src-tauri/
│       ├── tauri.conf.json       # externalBin: node, mongod; resources: { resources: resources }
│       ├── capabilities/         # shell:allow-execute scoped to the two sidecars
│       ├── dist/settings.html    # settings window
│       ├── src/{lib.rs,settings.rs}  # tray + supervisor: mongod → backend
│       └── binaries/ resources/  # generated by prepare-sidecars.mjs
└── frontend/
    ├── dist/{index.html,settings.html}  # loading + settings pages
    └── src-tauri/
        ├── tauri.conf.json       # externalBin: node; resources: { resources: resources }
        ├── src/{lib.rs,settings.rs}  # supervisor: node UI server → navigate window
...
1. Build the system tray (Show status / Settings… / Open data folder / Quit
   Music Server). The status and settings windows start hidden.
2. On a background thread:
   ...
   - Spawn the backend `node` sidecar with `cwd = resources/backend/dist`,
     `args = ["index.js"]`, `env NODE_ENV=production, PORT=<backendPort>,
     MONGO_URI=mongodb://127.0.0.1:<mongoPort>, HTTPS_ENABLED, TLS_CERT_PATH,
     TLS_KEY_PATH`. The backend listens on `0.0.0.0:<backendPort>` (HTTP or
     HTTPS per the saved settings — HTTPS is on by default).
...
2. On a background thread, spawn the UI `node` sidecar with
   ...
   MUSICSERVER_API_BASE_URL=<backendUrl>, HTTPS_ENABLED, TLS_CERT_PATH,
   TLS_KEY_PATH`. (`NODE_ENV=production` is required so the custom server runs
   the production build instead of dev mode.)
...
- Bundles/installers land in `src-tauri/target/release/bundle/` (or
  `target/<triple>/release/bundle/` when `--target` is passed).
- CI: `.github/workflows/build-packages.yml` builds both packages for four
  architectures (`macos-x64`, `macos-arm64`, `win-x64`, `linux-x64`). macOS
  Intel is **cross-compiled on the Apple Silicon runner** (`macos-14`) since
  GitHub's Intel runners (`macos-13`) are being retired; each job sets
  `MUSICSERVER_TARGET` (so `prepare-sidecars` stages the right sidecars) and
  builds with an explicit `--target`. Windows ARM64 is excluded (no MongoDB
  build).
- On a `v*` tag push, a `release` job publishes the installers (`.dmg`,
  NSIS `-setup.exe`, `.deb`) to a GitHub Release.
...
- Native installers — the CI produces macOS `.dmg`, Windows NSIS `-setup.exe`,
  and Linux `.deb` (Tauri can also emit `.msi`/`.AppImage`/`.rpm` if enabled).
